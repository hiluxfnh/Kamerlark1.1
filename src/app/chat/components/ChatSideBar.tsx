...
const ChatSideBar = ({ chatRoomId, setChatRoomId, setCurrentUser }) => {
    ...
    const fetch = async () => {
      if (user) {
        ...
        const filteredData = data.docs.filter((doc) =>
          doc.data().userIds.includes(user.uid)
        );
  ...
  return (
    <div>
      ...
      {chatRooms.map((room) => (
        <button
          className="flex flex-row items-center justify-between border-b-2 p-2 cursor-pointer min-w-full max-w-full"
          onClick={() => {
        ...
        >
          ...
          <div>
            <p>
              {room?.time?.split(":")[0]}:{room?.time?.split(":")[1]}
            </p>
          </div>
        </button>
      ))}
